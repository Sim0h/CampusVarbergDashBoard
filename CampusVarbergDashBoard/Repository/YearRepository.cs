...
namespace CampusVarbergDashBoard.Repository
{
	public class YearRepository : IYearRepository
	{
        ...
        public async Task<IEnumerable<string>> GetSpecificTermAsync(string term)
        {
            using (var connection = GetConnection())
            {
                DateTime startDate;
                ...
                Console.WriteLine($"Querying term: {term}, StartDate: {startDate}, EndDate: {endDate}");

                string query = @"
                        SELECT Inlämnad AS Termin
                        FROM dbo.ExcelData
                        WHERE Inlämnad BETWEEN @StartDate AND @EndDate";

                var result = await connection.QueryAsync<string>(query, new { StartDate = startDate, EndDate = endDate });
                Console.WriteLine($"Retrieved {result.Count()} records for term: {term}");
